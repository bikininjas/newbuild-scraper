...
def render_summary_table(category_products, history, selected_products=None, debug_info=None):
    ...
    html.append('<table class="min-w-full glass-card rounded-xl shadow-2xl border border-slate-600 overflow-hidden">')
    ...
    for cat, products in category_products.items():
        ...
        row_html += TD_PRODUCT.format('<select onchange="switchComponent(\'' + cat + '\', this.value)">' + ''.join(['<option value="' + p["name"] + '"' + (' selected' if p["name"] == name else '') + '>' + p["name"] + '</option>' for p in products]) + '</select>')
        ...
        if debug_info and (cat, name) in debug_info:
            ...
            debug_html += "<li>Source: <a href='{}' target='_blank'>{}</a></li>".format(dbg.get('source_url','#'), dbg.get('source_url','#'))
...
def render_product_cards(product_prices, history, product_min_prices):
    ...
    for name, entries in product_prices.items():
        ...
        html.append('<div class="glass-card rounded-2xl shadow-2xl border border-slate-600 p-8 hover:shadow-cyan-500/10 transition-all duration-300">')
        ...
        if not history_entries.empty:
            ...
            for _, h in history_entries.iterrows():
                ...
                if timestamp is None or (isinstance(timestamp, float) and math.isnan(timestamp)) or (isinstance(timestamp, str) and (timestamp.strip() == "" or timestamp.strip().lower() == "nan")):
                    continue
